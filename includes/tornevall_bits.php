...
class TORNEVALL_DNSBL_BITS {
	...
	public function getBitArray( $bitValue = 0 ) {
		$returnBitList = array();
		if ( $this->isBit( self::BIT_REPORTED, $bitValue ) ) {
			$returnBitList[] = "BIT_REPORTED";
		}
		if ( $this->isBit( self::BIT_CONFIRMED, $bitValue ) ) {
			$returnBitList[] = "BIT_CONFIRMED";
		}
		if ( $this->isBit( self::BIT_FRAUDBL, $bitValue ) ) {
			$returnBitList[] = "BIT_FRAUDBL";
		}
		if ( $this->isBit( self::BIT_EMPTY, $bitValue ) ) {
			$returnBitList[] = "BIT_EMPTY";
		}
		if ( $this->isBit( self::BIT_SPAM, $bitValue ) ) {
			$returnBitList[] = "BIT_SPAM";
		}
		if ( $this->isBit( self::BIT_SECOND_ENTRY, $bitValue ) ) {
			$returnBitList[] = "BIT_SECOND_ENTRY";
		}
		if ( $this->isBit( self::BIT_ABUSE, $bitValue ) ) {
			$returnBitList[] = "BIT_ABUSE";
		}
		if ( $this->isBit( self::BIT_DIFFERENT_STATE, $bitValue ) ) {
			$returnBitList[] = "BIT_DIFFERENT_STATE";
		}
		...
		for ( $bitCount = count( $bitArray[0] ); $bitCount >= 0; $bitCount -- ) {
			if ( isset( $bitArray[0][ $bitCount ] ) ) {
				if ( $matchWith & pow( 2, $bitCount ) ) {
					if ( $requestedBitValue == pow( 2, $bitCount ) ) {
						return true;
					}
				}
			}
