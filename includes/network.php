...
if (!class_exists('MODULE_NETWORK') && !class_exists('Tornevall_WP_DNSBL\MODULE_NETWORK')) {
    ...
    class MODULE_NETWORK
    {
        ...
        public function getUrlDomain($requestedUrlHost = '', $validateHost = false)
        {
            ...
            if ($validateHost || $this->alwaysResolveHostvalidation === true) {
                ...
                if (filter_var($requestedUrlHost, FILTER_VALIDATE_URL)) {
                    ...
                    if (!count($hostRecord)) {
                        //return array( null, null, null );
                        throw new \Exception(NETCURL_CURL_CLIENTNAME . " " . __FUNCTION__ . " exception: Host validation failed",
        ...
        public function setCookie($name = '', $value = '', $expire = '')
        {
            ...
            return setcookie($this->cookieDefaultPrefix . $name, $value, $expire, $this->cookieDefaultPath,
                $this->cookieDefaultDomain, $this->cookieUseSecure);
        }
        ...
        public function getIpv6FromOctets(
            ...
            return @inet_ntop(pack('H*',
                implode("", array_reverse(explode(".", preg_replace("/\.ip6\.arpa$|\.ip\.int$/", '', $arpaOctets))))));
        }
