...
use TorneLIB\TORNEVALL_DNSBL_BITS;
use Tornevall_WP_LIBDNSBL\TorneLIB_Network;
use Tornevall_WP_LIBDNSBL\TorneLIB_Network_IP;
...
class TornevallDNSBL {
	...
	function __construct() {
		...
		foreach ( $this->filterOn as $filterKey ) {
			$compatFilters[] = "BIT_" . strtoupper( $filterKey );
		}
		$this->filterOn = $compatFilters;
		$this->BitClass = new TORNEVALL_DNSBL_BITS();
		$this->NETLIB   = new TorneLIB_Network();
		// Used for testing
	...
	private function resolveBlacklist( $ipAddr, $getListedTypes = false, $getBitValue = false ) {
		$listedAt = array();
		$lastBit  = array();
		if ( $this->NETLIB->getArpaFromAddr( $ipAddr, true ) !== TorneLIB_Network_IP::IPTYPE_NONE ) {
		...
		if ( $getBitValue && count( $lastBit ) == 4 ) {
			return $lastBit[3];
		}
	...
	private function isListed( $ipaddr, $getBitValue = false ) {
		...
		if ( isset( $this->ipBits['dnsbl.tornevall.org'] ) && count( $this->ipBits['dnsbl.tornevall.org'] ) ) {
	...
	public function testip( $remoteAddressRequest = '' ) {
		...
		if ( isset( $_REQUEST['showresponse'] ) ) {
			echo __( 'Requested to show dnsbl response instead of showing site.', 'tornevall_dnsbl' ) . "<br>";
			echo __( 'Resolved address is: ', 'tornevall_dnsbl' ) . $addr . "<br>";
			echo __( 'Response is: ', 'tornevall_dnsbl' ) . ( ! empty( $isListed ) ? $isListed : __( 'Not set' ) );
			die();
		}
		if ( ! is_null( $isListed ) ) {
			$dnsblReact = false;
