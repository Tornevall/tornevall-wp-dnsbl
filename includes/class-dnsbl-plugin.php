...
class Plugin
{
    ...
    public static function filterRuntimeTranslations($translation, $text, $domain)
    {
        ...
        $fallbacks = [
                ...
                'CIDR delist request timed out while waiting for Tools. Already submitted delete operations may still finish in the background, so re-check the listed hit list before retrying immediately.' => 'CIDR-avlistningen timeoutade medan svar från Tools inväntades. Redan skickade delete-operationer kan fortfarande bli klara i bakgrunden, så kontrollera träfflistan igen innan du försöker direkt på nytt.',
                'CIDR delist request accepted.' => 'CIDR-avlistningsbegäran accepterades.',
                'Check CIDR locally' => 'Kontrollera CIDR lokalt',
                'Checking CIDR locally…' => 'Kontrollerar CIDR lokalt…',
                'Starting local CIDR scan…' => 'Startar lokal CIDR-skanning…',
                'The local CIDR check could not be completed.' => 'Den lokala CIDR-kontrollen kunde inte slutföras.',
                'Run the local CIDR check first. CIDR delist is only enabled after that scan has found at least one listed address in the block.' => 'Kör den lokala CIDR-kontrollen först. CIDR-avlistning aktiveras först när den kontrollen har hittat minst en listad adress i blocket.',
                'Enter a CIDR range like 203.0.113.0/24 before running the local check.' => 'Ange ett CIDR-intervall som 203.0.113.0/24 innan du kör den lokala kontrollen.',
                'CIDR delist is ready. Listed addresses were found in the block.' => 'CIDR-avlistning är klar. Listade adresser hittades i blocket.',
                'No listed addresses were found in that CIDR block.' => 'Inga listade adresser hittades i det CIDR-blocket.',
                'The scan uses the configured blacklist zones in small local batches so the resolver side is not flooded.' => 'Skanningen använder de konfigurerade blacklist-zonerna i små lokala batchar så att resolversidan inte överbelastas.',
            ...
            <p>
                <?php if ($requestUrl !== '') { ?>
                    <a class="button button-secondary" target="_blank" rel="noopener noreferrer"
                       href="<?php echo esc_url($requestUrl); ?>">
        ...
        <form class="tornevall-dnsbl-removal-form" data-tornevall-dnsbl-removal-form="1"
            ...
            <?php } elseif ($showOperationFields) { ?>
                ...
                <p style="margin-bottom:.85rem;">
                    <strong><?php echo esc_html__('Operation', 'tornevall-networks-dnsbl-implementation'); ?>
                        :</strong> <?php echo esc_html((string)($availableActions[$defaultAction] ?? $defaultAction)); ?>
                </p>
            <?php } else { ?>
                <input type="hidden" name="operation"
                       value="<?php echo esc_attr($checkerMode ? 'check' : $defaultAction); ?>">
            <?php } ?>

            <p>
                <label><?php echo esc_html__('IP address', 'tornevall-networks-dnsbl-implementation'); ?><br>
                    <input type="text" name="ip" value="<?php echo esc_attr($prefillIp); ?>" placeholder="203.0.113.10"
                           required style="min-width:260px;" data-removal-ip
                           data-prefill-ip="<?php echo esc_attr($prefillIp); ?>"/>
                </label>
            </p>

            <?php if ($checkerMode && $canUseAdvancedCidr) { ?>
                <p style="margin:0 0 .65rem 0;">
                    <button type="button" class="button button-secondary" data-removal-advanced-toggle
                            style="display:none;">
                        <?php echo esc_html__('Advanced', 'tornevall-networks-dnsbl-implementation'); ?>
                    </button>
                </p>
                <div data-removal-advanced
                     style="display:none; margin:0 0 .8rem 0; padding:.65rem .75rem; border:1px solid #dbeafe; border-radius:8px; background:#eff6ff;">
                    <p style="margin:0 0 .45rem 0; color:#1e3a8a;">
                    ...
                    <input type="hidden" name="cidr_scan_token" value="" data-removal-cidr-scan-token/>
                    <p style="margin:.6rem 0 0 0; display:flex; gap:.5rem; align-items:center; flex-wrap:wrap;">
                        <button type="button" class="button button-secondary" data-removal-cidr-check>
                            <?php echo esc_html__('Check CIDR locally', 'tornevall-networks-dnsbl-implementation'); ?>
                        </button>
                    </p>
                    <p style="margin:.45rem 0 0 0; color:#1e3a8a; font-size:.88em;">
                        <?php echo esc_html__('The scan uses the configured blacklist zones in small local batches so the resolver side is not flooded.', 'tornevall-networks-dnsbl-implementation'); ?>
                    </p>
                    <div data-removal-cidr-progress-wrap style="display:none; margin:.65rem 0 0 0;">
                        <div style="width:100%; height:12px; border-radius:999px; overflow:hidden; background:#dbeafe; border:1px solid #bfdbfe;">
                            <div data-removal-cidr-progress-bar
                                 style="width:0%; height:100%; background:#2563eb;"></div>
                        </div>
                        <div data-removal-cidr-progress-text
                             style="margin:.35rem 0 0 0; font-size:.92em; color:#1e3a8a;"></div>
                    </div>
                    <div data-removal-cidr-status
                         style="display:none; margin-top:.65rem; padding:.55rem .65rem; border-radius:6px; border:1px solid #bfdbfe;"></div>
                    <div data-removal-cidr-hitlist
                         style="display:none; margin-top:.65rem; padding:.6rem .7rem; border-radius:6px; border:1px solid #bfdbfe; background:#eff6ff; color:#1e3a8a;">
                        <p style="margin:0 0 .4rem 0;">
                            <strong><?php echo esc_html__('Listed addresses found in this CIDR block', 'tornevall-networks-dnsbl-implementation'); ?></strong>
                        </p>
                        <div data-removal-cidr-hitlist-summary style="margin:0 0 .45rem 0; font-size:.92em;"></div>
                        <ul data-removal-cidr-hitlist-items
                            style="margin:0; padding-left:1.1rem; max-height:12rem; overflow:auto;"></ul>
                    </div>
                </div>
            ...
            <?php if ($showBitmaskField) { ?>
                <p>
                    <label><?php echo esc_html__('Bitmask', 'tornevall-networks-dnsbl-implementation'); ?><br>
                        <input type="number" name="bitmask" min="0" max="255"
                               value="<?php echo esc_attr((string)((int)$atts['bitmask'])); ?>" required/>
                    </label>
                </p>
            <?php } else { ?>
            ...
            <?php if ($showOperationFields) { ?>
                <p data-removal-old-bitmask-wrap
                   style="display:<?php echo $defaultActionIsUpdate ? 'block' : 'none'; ?>;">
                    <label><?php echo esc_html__('Old bitmask (required for update)', 'tornevall-networks-dnsbl-implementation'); ?>
                        <br>
                        <input type="number" name="old_bitmask" min="0" max="255"
                               value="0" <?php echo $defaultActionIsUpdate ? 'required' : ''; ?> />
                    </label>
                </p>
            <?php } else { ?>
            ...
            <?php } else { ?>
                <input type="hidden" name="publication_type"
                       value="<?php echo esc_attr((string)$atts['publication_type']); ?>">
            <?php } ?>

            <?php if ($showTtlField) { ?>
                <p>
                    <label><?php echo esc_html__('TTL (seconds)', 'tornevall-networks-dnsbl-implementation'); ?><br>
                        <input type="number" name="ttl" min="300" max="86400"
                               value="<?php echo esc_attr((string)max(300, (int)$atts['ttl'])); ?>"/>
                    </label>
                </p>
            <?php } else { ?>
            ...
            <p style="margin:0; display:flex; gap:.5rem; align-items:center; flex-wrap:wrap;">
                ...
                <?php if ($checkerMode) { ?>
                    <button type="button" class="button button-secondary" data-removal-delist
                            style="display:none; opacity:.95;" disabled>
                        <?php echo esc_html__('Delist', 'tornevall-networks-dnsbl-implementation'); ?>
                    </button>
                    <button type="button" class="button button-secondary" data-removal-reset>
                        <?php echo esc_html__('Reset', 'tornevall-networks-dnsbl-implementation'); ?>
                    </button>
                <?php } ?>
            </p>

            <div data-removal-busy
                 style="display:none; margin-top:.65rem; align-items:center; gap:.55rem; color:#1e3a8a;"
                 aria-live="polite">
                <span aria-hidden="true"
                      style="display:inline-block; width:16px; height:16px; border:2px solid #bfdbfe; border-top-color:#2563eb; border-radius:999px; animation:tornevall-dnsbl-spin .8s linear infinite;"></span>
                <span data-removal-busy-text><?php echo esc_html__('Working…', 'tornevall-networks-dnsbl-implementation'); ?></span>
            </div>

            <div data-removal-result
                 style="display:none; margin-top:.85rem; padding:.65rem .75rem; border-radius:6px; border:1px solid #d1d5db;"></div>
        </form>
        <?php
    ...
    private static function renderRemovalAccessNotice(string $message, array $permissionSummary = []): string
    {
        ...
        ob_start();
        ?>
        <div class="tornevall-dnsbl-removal-access-notice"
             style="margin:0 0 1rem 0; padding:1rem 1.1rem; border-radius:10px; border:1px solid #fdba74; background:#fff7ed; color:#9a3412;">
    ...
    private static function enqueueRemovalFormAssets(): void
    {
        ...
        wp_localize_script('tornevall-dnsbl-removal-form', 'tornevallDnsblRemovalForm', [
                ...
                'cidrFailedText' => __('The local CIDR check could not be completed.', 'tornevall-networks-dnsbl-implementation'),
                'cidrScanRequiredText' => __('Run the local CIDR check first. CIDR delist is only enabled after that scan has found at least one listed address in the block.', 'tornevall-networks-dnsbl-implementation'),
                'cidrEmptyText' => __('Enter a CIDR range like 203.0.113.0/24 before running the local check.', 'tornevall-networks-dnsbl-implementation'),
                'cidrReadyText' => __('CIDR delist is ready. Listed addresses were found in the block.', 'tornevall-networks-dnsbl-implementation'),
                'cidrEmptyResultText' => __('No listed addresses were found in that CIDR block.', 'tornevall-networks-dnsbl-implementation'),
                'cidrHitListTitleText' => __('Listed addresses found in this CIDR block', 'tornevall-networks-dnsbl-implementation'),
    ...
    public static function handleRemovalFormAjax(): void
    {
        ...
        if (self::requestBoolean($_POST['cidr_scan'] ?? false)) {
            self::handleRemovalCidrScanAjax();
        }
        ...
        $allowCidrOnlyDelete = $cidrRangeRaw !== '' && !$checkOnly && !$backgroundApiCheck && $confirmedListed;

        $ip = isset($_POST['ip']) ? trim(sanitize_text_field(wp_unslash($_POST['ip']))) : '';
        if ($ip !== '' && strpos($ip, '/') !== false) {
            wp_send_json_error([
                    'ok' => false,
                    'message' => __('A CIDR range was entered in the IP field. The first field only accepts one IP address. Open Advanced and place the CIDR range there instead.', 'tornevall-networks-dnsbl-implementation'),
                    'needs_cidr_handoff' => true,
            ], 422);
        }
        if (!filter_var($ip, FILTER_VALIDATE_IP) && !$allowCidrOnlyDelete) {
            wp_send_json_error([
                    'ok' => false,
        ...
        if ($checkerMode && filter_var($ip, FILTER_VALIDATE_IP)) {
        ...
        $cidrScanToken = isset($_POST['cidr_scan_token']) ? trim(sanitize_text_field(wp_unslash($_POST['cidr_scan_token']))) : '';
        $cidrRange = '';
        $targetIps = [$ip];
        $cidrListedTargets = [];
        if ($operation === 'delete' && $cidrRangeRaw !== '') {
            ...
            $cidrApproval = self::getStoredCidrScanApproval($cidrScanToken);
            if (!count($cidrApproval)
                    || (string)($cidrApproval['cidr'] ?? '') !== $cidrRange
                    || (int)($cidrApproval['listed_count'] ?? 0) < 1) {
                wp_send_json_error([
                        'ok' => false,
                        'message' => __('Run the local CIDR check first. CIDR delist stays disabled until at least one listed address is found in that block.', 'tornevall-networks-dnsbl-implementation'),
                ], 409);
            }

            $cidrListedTargets = array_values(array_unique(array_filter(array_map(static function ($scanIp) {
                return filter_var($scanIp, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) ? (string)$scanIp : '';
            }, (array)($cidrApproval['listed_ips'] ?? [])))));
            $targetIps = count($cidrListedTargets)
                    ? $cidrListedTargets
                    : self::expandIpv4Cidr($cidrRange);
            if (!count($targetIps)) {
                wp_send_json_error([
                        'ok' => false,
        ...
        if ($operation === 'add') {
            ...
            if ($cidrRange !== '') {
                $apiResult = $client->removeIpsSequentially($targetIps, $bitmask, $publicationType, $requestOptions);
                if (is_array($apiResult['body'] ?? null)) {
                    $apiResult['body']['listed_target_count'] = count($cidrListedTargets);
                    $apiResult['body']['cidr_range'] = $cidrRange;
                }
            } else {
        ...
        $payload = [
                ...
                'listed_target_count' => count($cidrListedTargets),
                'timed_out' => !empty($apiResult['timed_out']) || !empty($body['timed_out']),
        ];
    ...
    {
        $permissionSummary = self::getWritePermissionSummary();
        if (!self::canUseAdvancedCidr($permissionSummary)) {
            wp_send_json_error([
                    'ok' => false,
                    'message' => __('CIDR delist is not allowed for this token. Request approval via Tools before using Advanced mode.', 'tornevall-networks-dnsbl-implementation'),
                    'request_url' => untrailingslashit(self::toolsBaseUrl()) . '/dnsbl/cidr/request',
        ...
        $scanId = isset($_POST['cidr_scan_id']) ? trim(sanitize_text_field(wp_unslash($_POST['cidr_scan_id']))) : '';
        $state = [];

        if ($scanId !== '') {
            $state = self::getStoredCidrScanState($scanId);
            if (!count($state)) {
                wp_send_json_error([
                        'ok' => false,
                        'message' => __('The CIDR scan session expired. Start the local check again.', 'tornevall-networks-dnsbl-implementation'),
                ], 410);
            }
        } else {
            $cidrRangeRaw = isset($_POST['cidr_range']) ? trim(sanitize_text_field(wp_unslash($_POST['cidr_range']))) : '';
            ...
            $cidr = (string)($cidrValidation['cidr'] ?? '');
            $ips = self::expandIpv4Cidr($cidr);
            if (!count($ips)) {
                wp_send_json_error([
                        'ok' => false,
                        'message' => __('Could not resolve the requested CIDR range into addresses.', 'tornevall-networks-dnsbl-implementation'),
        ...
        $allIps = isset($state['ips']) && is_array($state['ips']) ? array_values(array_filter($state['ips'], 'is_string')) : [];
        $listedIps = isset($state['listed_ips']) && is_array($state['listed_ips']) ? array_values(array_filter($state['listed_ips'], 'is_string')) : [];
        $listedMap = array_fill_keys($listedIps, true);
        $batchIps = array_slice($allIps, $processed, self::CIDR_SCAN_BATCH_SIZE);

        foreach ($batchIps as $batchIp) {
            $lookup = self::buildLookupResult($batchIp);
            if (!empty($lookup['listed'])) {
                $listedMap[$batchIp] = true;
            }
        }

        $processed += count($batchIps);
        $listedIps = array_values(array_keys($listedMap));
        $listedCount = count($listedIps);
        $complete = $processed >= $total;
        $approvalToken = '';
        $cidr = (string)($state['cidr'] ?? '');

        if ($complete) {
            self::deleteStoredCidrScanState((string)($state['scan_id'] ?? ''));
            if ($listedCount > 0) {
                $approvalToken = self::storeCidrScanApproval($cidr, $listedIps);
            }
        } else {
            $state['processed'] = $processed;
            $state['listed_ips'] = $listedIps;
            $state['listed_count'] = $listedCount;
            $state['total'] = $total;
            self::storeCidrScanState($state);
        }
        ...
        $message = $complete
                ? ($listedCount > 0
                        ? sprintf(
                                __('Local CIDR scan finished. %1$d of %2$d addresses in %3$s are currently listed. CIDR delist can now be submitted.', 'tornevall-networks-dnsbl-implementation'),
                                $listedCount,
                                $total,
                                $cidr
                        )
                        : sprintf(
                                __('Local CIDR scan finished. No listed addresses were found in %s.', 'tornevall-networks-dnsbl-implementation'),
                                $cidr
                        ))
                : sprintf(
                        __('Local CIDR scan is running… %1$d of %2$d addresses checked.', 'tornevall-networks-dnsbl-implementation'),
                        $processed,
                        $total
                );
        ...
        wp_send_json_success([
                ...
                'listed_ips' => $listedIps,
                'resolver_host_count' => count(self::getResolverHosts()),
                'batch_size' => self::CIDR_SCAN_BATCH_SIZE,
                'cidr_scan_token' => $approvalToken,
                'can_submit_cidr_delete' => $complete && $listedCount > 0,
        ]);
    }
     ...
     */
    private static function getStoredCidrScanState(string $scanId): array
    {
        if ($scanId === '') {
            return [];
        }

        $state = get_transient(self::getCidrScanStateCacheKey($scanId));

        return is_array($state) ? $state : [];
    }

    private static function getCidrScanStateCacheKey(string $scanId): string
    {
        return 'tornevall_dnsbl_cidr_scan_state_' . md5($scanId);
    }
    ...
    private static function deleteStoredCidrScanState(string $scanId): void
    {
        if ($scanId === '') {
            return;
        }

        delete_transient(self::getCidrScanStateCacheKey($scanId));
    }

    /**
     * @param list<string> $listedIps
     */
    private static function storeCidrScanApproval(string $cidr, array $listedIps): string
    {
        $approvalToken = wp_generate_uuid4();
        $listedIps = array_values(array_unique(array_filter(array_map(static function ($ip) {
            return filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) ? (string)$ip : '';
        }, $listedIps))));

        set_transient(self::getCidrScanApprovalCacheKey($approvalToken), [
                'cidr' => $cidr,
                'listed_ips' => $listedIps,
                'listed_count' => count($listedIps),
                'created_at' => time(),
        ], self::CIDR_SCAN_APPROVAL_TTL);

        return $approvalToken;
    }

    private static function getCidrScanApprovalCacheKey(string $approvalToken): string
    {
        return 'tornevall_dnsbl_cidr_scan_approval_' . md5($approvalToken);
    }

    /**
     * @param array<string,mixed> $state
     */
    private static function storeCidrScanState(array $state): void
    {
        $scanId = isset($state['scan_id']) ? trim((string)$state['scan_id']) : '';
        if ($scanId === '') {
            return;
        }

        set_transient(self::getCidrScanStateCacheKey($scanId), $state, self::CIDR_SCAN_STATE_TTL);
    }
     ...
     * @return array<string,mixed>
     */
    private static function getStoredCidrScanApproval(string $approvalToken): array
    {
        if ($approvalToken === '') {
            return [];
        }

        $approval = get_transient(self::getCidrScanApprovalCacheKey($approvalToken));
