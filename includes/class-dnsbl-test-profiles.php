...
class TestProfiles
{
    private const ENTRIES_OPTION = 'tornevall_dnsbl_test_profiles_entries';
    private const RUNTIME_OPTION = 'tornevall_dnsbl_test_profiles_runtime';
    private const PAGE_SLUG = 'tornevallDnsblTestProfiles';
    private const NONCE_ACTION = 'tornevall_dnsbl_test_profiles_admin';
    ...
    public static function registerMenu(): void
    {
        add_submenu_page(
            'tornevallDnsblMenu',
            __('Test data', 'tornevall-networks-dnsbl-implementation'),
            __('Test data', 'tornevall-networks-dnsbl-implementation'),
            'manage_options',
            self::PAGE_SLUG,
    ...
    /**
     * Resolve a configured test identifier.
    ...
    public static function renderPage(): void
    {
        ...
        ?>
        <div class="wrap">
            <h1><?php echo esc_html__('Test data', 'tornevall-networks-dnsbl-implementation'); ?></h1>
            <p class="description" style="max-width:900px;">
                <?php echo esc_html__('Configurable test profiles stored in WordPress data. No test identities are hard-coded into the plugin.', 'tornevall-networks-dnsbl-implementation'); ?>
            </p>

            <?php if ($testResult !== null) { ?>
                <div class="notice notice-info inline" style="margin:16px 0;"><p>
                    <?php if (empty($testResult['found'])) { ?>
                        <?php echo esc_html__('No configured profile matched the identifier.', 'tornevall-networks-dnsbl-implementation'); ?>
                    <?php } else { ?>
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(340px, 1fr)); gap:16px; margin-top:16px; align-items:start;">
                <div class="postbox"><div class="inside">
                    <h2 style="margin-top:0;"><?php echo esc_html__('Add or update test profile', 'tornevall-networks-dnsbl-implementation'); ?></h2>
                    <form method="post">
                        <?php wp_nonce_field(self::NONCE_ACTION); ?>
                        <input type="hidden" name="test_profile_operation" value="save">
                        ...
                        <?php submit_button(__('Save test profile', 'tornevall-networks-dnsbl-implementation'), 'primary', 'submit', false); ?>
                    </form>
                </div></div>

                <div class="postbox"><div class="inside">
                    <h2 style="margin-top:0;"><?php echo esc_html__('Run test lookup', 'tornevall-networks-dnsbl-implementation'); ?></h2>
                    ...
                    <form method="post">
                        <?php wp_nonce_field(self::NONCE_ACTION); ?>
                        <input type="hidden" name="test_profile_operation" value="test">
                        <p><input type="text" class="regular-text" name="identifier" required></p>
                        <?php submit_button(__('Run test lookup', 'tornevall-networks-dnsbl-implementation'), 'secondary', 'submit', false); ?>
                    </form>
                    <form method="post" style="margin-top:16px;">
                        <?php wp_nonce_field(self::NONCE_ACTION); ?>
                        <input type="hidden" name="test_profile_operation" value="reset_runtime">
                        <?php submit_button(__('Reset all test timers', 'tornevall-networks-dnsbl-implementation'), 'secondary', 'submit', false); ?>
                    </form>
                </div></div>
            </div>

            <div class="postbox" style="margin-top:16px;"><div class="inside">
                <h2 style="margin-top:0;"><?php echo esc_html__('Configured profiles', 'tornevall-networks-dnsbl-implementation'); ?></h2>
                <?php if (!count($entries)) { ?>
                    <p class="description"><?php echo esc_html__('No test profiles configured yet.', 'tornevall-networks-dnsbl-implementation'); ?></p>
                <?php } else { ?>
                    <table class="widefat striped">
                        ...
                        <?php foreach ($entries as $entry) { ?>
                            <tr>
                                ...
                                <td><form method="post">
                                    <?php wp_nonce_field(self::NONCE_ACTION); ?>
                                    <input type="hidden" name="test_profile_operation" value="delete">
                                    <input type="hidden" name="entry_id" value="<?php echo esc_attr((string)$entry['id']); ?>">
                                    <button class="button button-small" type="submit" onclick="return confirm('<?php echo esc_js(__('Delete this test profile?', 'tornevall-networks-dnsbl-implementation')); ?>');"><?php echo esc_html__('Delete', 'tornevall-networks-dnsbl-implementation'); ?></button>
                                </form></td>
                            </tr>
                        <?php } ?>
                        </tbody>
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(340px, 1fr)); gap:16px; margin-top:16px; align-items:start;">
                <div class="postbox"><div class="inside">
                    ...
                    <form method="post">
                        <?php wp_nonce_field(self::NONCE_ACTION); ?>
                        <input type="hidden" name="test_profile_operation" value="import">
    ...
    {
        if (($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST' || empty($_POST['test_profile_operation'])) {
            return null;
        }

        check_admin_referer(self::NONCE_ACTION);
        $operation = sanitize_text_field(wp_unslash($_POST['test_profile_operation']));
    ...
    /** @param array<string,mixed> $rawEntry @return array<string,mixed>|null */
